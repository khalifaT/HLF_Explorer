...
function clearContainers() {
  ...
  docker rm -f $(docker ps -aq --filter name='explorer*') 2>/dev/null || true
}
...
function deployExplorer() {
    #rm -rf ./explorer/connection-profile/test-network.json
    #cp -rf organizations/peerOrganizations/org1.$ORG_NAME.com/connection-org1.json ./explorer/connection-profile/test-network.json
    cp -f organizations/peerOrganizations/org1.$ORG_NAME.com/users/Admin@org1.$ORG_NAME.com/msp/keystore/* organizations/peerOrganizations/org1.$ORG_NAME.com/users/Admin@org1.$ORG_NAME.com/msp/keystore/priv_sk
    cd explorer; docker-compose up -d; cd ..
...
elif [ "$MODE" == "deployExplorer" ]; then
  infoln "Deploying of hyperledger explorer"
elif [ "$MODE" == "down" ]; then
